...
type UseSubmitCodeProps = {
  ...
  setSubmissionPending: React.Dispatch<
    React.SetStateAction<SubmissionPendingObj>
  >;
  setSubmissionResults: React.Dispatch<
    React.SetStateAction<SubmissionData | null>
  >;
...
export function useSubmitCode({
  ...
  const [submissionTokens, setSubmissionTokens] = useState<
        ...
        try {
          if (type === SubmissionType.RUN && submissionTokens) {
            const resultResponse = await fetch(
              `${process.env.NEXT_PUBLIC_BASE_URL}/api/check/run`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
                ...
                body: JSON.stringify({
                  problemId: problemId,
                  testCases: testCases,
                  submissionTokens: submissionTokens.map(
            ...
            const resultResponse = await fetch(
              `${process.env.NEXT_PUBLIC_BASE_URL}/api/check/submit/${submissionId}`,
            );
            const submissionResult = await resultResponse.json();
            if (!resultResponse.ok) {
              throw new Error(submissionResult.msg);
