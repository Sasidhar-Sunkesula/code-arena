...
export async function getCurrentContests(userId?: string) {
    try {
        ...
        const registeredContests = contests.filter(contest => contest.isRegistered).map((({ users, ...rest }) => rest))
        const unregisteredContests = contests.filter(contest => !contest.isRegistered).map(({ users, ...rest }) => rest)
