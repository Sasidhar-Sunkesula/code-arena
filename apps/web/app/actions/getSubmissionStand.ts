...
export async function getSubmissionStand(
  problemId: number,
  contestId?: number,
) {
  try {
    ...
    const sortedData = submissionData.sort(
      (a, b) => (a.runTime ?? 0) - (b.runTime ?? 0),
    );
    ...
    const runtimePercentage = Object.keys(runtimeFrequency).map((runTime) => ({
      ...
      percentage: parseFloat(
        (
          ((runtimeFrequency[parseFloat(runTime)] ?? 0) / totalSubmissions) *
          100
        ).toFixed(2),
      ),
    }));
    ...
    const memoryPercentage = Object.keys(memoryFrequency).map((memory) => ({
      ...
      percentage: parseFloat(
        (
          ((memoryFrequency[parseFloat(memory)] ?? 0) / totalSubmissions) *
          100
        ).toFixed(2),
      ),
    }));
    ...
    return {
      ...
      msg:
        error instanceof Error
          ? error.message
          : "Error while fetching submissions data",
    };
  }
}
