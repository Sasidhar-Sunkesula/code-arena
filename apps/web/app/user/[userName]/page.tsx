...
export default async function Profile({
  ...
  const { solvedProblems, totalProblems, totalProblemsByLevel } =
    solvedProblemsData;

  // Reduce the solved problems to count by difficulty level
  const solvedProblemsByLevel: SolvedProblemsByLevel = solvedProblems.reduce(
    (acc, curr) => {
    ...
    <div className="grid grid-cols-5 gap-x-5 px-8 py-3">
      ...
      <div className="col-span-4 space-y-4">
        <SubmissionsLineChart
          chartData={submissionsByMonth.submissionsByMonth}
        />
