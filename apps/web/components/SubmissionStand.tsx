...
export function SubmissionStand({
  chartData,
  currentSubmission,
}: {
  chartData: ChartData[];
  currentSubmission: { [K in keyof ChartConfig]: number };
}) {
  const [activeChart, setActiveChart] = useState<keyof ChartConfig>("runTime");
  const percentageKey =
    activeChart === "runTime" ? "runTimePercentage" : "memoryPercentage";
  // Calculate beatsInRunTime and beatsInMemory
  const totalSubmissions = chartData.length;
  const slowerSubmissions = chartData.filter(
    (data) => data.runTime > currentSubmission.runTime,
  ).length;
  const beatsInRunTime = ((slowerSubmissions / totalSubmissions) * 100).toFixed(
    2,
  );

  const higherMemorySubmissions = chartData.filter(
    (data) => data.memory > currentSubmission.memory,
  ).length;
  const beatsInMemory = (
    (higherMemorySubmissions / totalSubmissions) *
    100
  ).toFixed(2);
