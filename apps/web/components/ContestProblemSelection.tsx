...
export function ContestProblemSelection({
  control,
  setSelectedProblems,
}: ContestProblemSelectionProps) {
      ...
      try {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Unable to get the search results",
        );
      } finally {
        setLoading(false);
      }
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
              ...
              {searchKey.trim() &&
                ...
                searchResults.map((result) => {
                  ...
                  return (
                    <div
                      key={result.id}
                      className="px-8 py-1 flex items-center border justify-between"
                    >
          ...
          <FormDescription>
            Select problems for the contest minimum 1 problem should be
            selected.
          </FormDescription>
          <FormMessage />
        </FormItem>
