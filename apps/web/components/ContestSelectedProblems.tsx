...
export function ContestSelectedProblems({ watch, selectedProblems, setValue, setSelectedProblems }: ContestSelectedProblemsProps) {
    ...
    return (
        <div className="space-y-3">
            ...
            >
                {!searchKey.trim() && filteredProblems.length === 0 && <div className="text-sm">Search something at first</div>}
                {filteredProblems.length > 0 && (
                    filteredProblems.map(problem => (
                        <div key={problem.id} className="flex justify-between items-start w-full border gap-x-1">
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    const newProblemIds = problemIds.filter(id => id !== problem.id);
                                    const newSelectedProblems = selectedProblems.filter(p => p.id !== problem.id);
                ...
                {searchKey.trim() && filteredProblems.length === 0 && <div className="text-sm">No results found</div>}
            </div>
        </div>
    );
}