...
export function CodeEditor({
  ...
  userType?: string;
  tempId?: string;
  boilerPlates: BoilerPlateWithLanguage[];
  contestId?: string;
  problemId: number;
}) {
  const [selectedLanguage, setSelectedLanguage] = useState(
    boilerPlates[0]?.language.monacoName || "",
  );
  const boilerPlateOfSelectedLang = boilerPlates.find(
    (item) => item.language.monacoName === selectedLanguage,
  );
  const [fullCode, setFullCode] = useState<string>(
    boilerPlateOfSelectedLang?.initialFunction || "",
  );
  ...
  useEffect(() => {
    async function fetchSubmissionsData() {
      try {
        ...
        const data = await getSubmissionStand(
          problemId,
          contestId ? parseInt(contestId) : undefined,
        );
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Unable to fetch submissions data",
        );
      } finally {
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <TabsList>
          <TabsTrigger value="editor" className="md:w-[125px]">
            Editor
          </TabsTrigger>
          <TabsTrigger value="submissions" className="md:w-[125px]">
            Submissions
          </TabsTrigger>
          {submissionsData && (
            <TabsTrigger value="lastSubmission">
        ...
        <TabsContent value="submissions">
          {boilerPlateOfSelectedLang && (
            <ProblemSubmissions
              ...
              contestId={
                contestId && !isNaN(parseInt(contestId))
                  ? parseInt(contestId)
                  : undefined
              }
            />
      ...
      {boilerPlateOfSelectedLang?.languageId ||
