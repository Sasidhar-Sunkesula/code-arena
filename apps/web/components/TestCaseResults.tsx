...
export function TestCaseResults({
  ...
  submissionResults: SubmissionData;
}) {
  const syntaxError = submissionResults.testCaseResults.find(
    (result) => result.stderr && result.stderr.includes("SyntaxError"),
  );
  ...
  return (
    <div className="flex flex-col gap-y-3">
      <div className="flex items-center justify-between py-1">
        <div
          className={`font-semibold text-xl ${submissionResults.status === SubmissionStatus.Accepted ? "text-green-600" : "text-red-600"}`}
        >
          {SubmissionStatusDisplay[submissionResults.status]}
        </div>
        <div className="flex items-center gap-x-4">
          <div className="text-sm flex items-center gap-x-1">
            <Timer className="w-3" /> {formatRunTime(submissionResults.runTime)}{" "}
            ms
          </div>
          <div className="text-sm flex items-center gap-x-1">
          ...
          >
            {result.stderr && result.status !== "Accepted" ? (
              <>
                <CodeBlock
                  title="Status"
                  content={SubmissionStatusDisplay[result.status]}
                />
