...
export function UserDetails({ id, createdAt, username, fullName, image, location }: {
    ...
    async function handleImageUpload() {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/upload-profile-pic`, {
                method: 'POST',
                body: formData,
            });
            ...
            <div className="relative">
                ...
                {canEdit && (
                    <>
                        {
                            profileImage ? (
                                <Button onClick={handleImageUpload} className="rounded-full bg-green-500 hover:bg-green-500 absolute right-0 bottom-1" size={"icon"}>
                                    <Check className="w-5 text-primary-foreground" />
                                </Button>
                            ) : (
                                <Label htmlFor="profile" className="rounded-full absolute right-0 bottom-1 cursor-pointer bg-secondary p-3">
                                    <CloudUpload className="w-4 h-4" />
                                </Label>
                            )
                        }
                        <Input onChange={handleFileChange} type="file" id="profile" className="hidden" />
                    </>
                )}
            </div>
            {
                !canEdit || !renderEdit ?
                    (
                        <div className="grid grid-cols-2 px-2 gap-y-2 gap-x-6 justify-between md:w-10/12 mx-auto items-center">
                            ...
                            <div className="text-sm col-span-1" suppressHydrationWarning>{createdAt.toLocaleDateString()}</div>

                            {canEdit && (
                                <div className="col-span-2 flex justify-center mt-2">
                                    <Button onClick={() => setRenderEdit(true)} size={"sm"}>Edit Profile</Button>
                                </div>
                            )}
                        </div>
                    )
                    : (
                        <form onSubmit={(e) => handleSubmit(e)} className="space-y-3">
                            <div>
                                <Label>Name</Label>
                                <Input value={userDetails.fullName ?? ""} onChange={(e) => setUserDetails(prev => ({ ...prev, fullName: e.target.value }))} />
                            </div>
                            <div>
                                <Label>Username</Label>
                                <Input value={userDetails.username} onChange={(e) => setUserDetails(prev => ({ ...prev, username: e.target.value }))} />
                            </div>
                            <div>
                                <Label>Location</Label>
                                <Input value={userDetails.location ?? ""} onChange={(e) => setUserDetails(prev => ({ ...prev, location: e.target.value }))} />
                            </div>
                            <div className="flex items-center gap-x-5">
                                <Button type="submit">Submit</Button>
                                <Button onClick={() => setRenderEdit(false)} type="button">Cancel</Button>
                            </div>
                        </form>
                    )
            }
